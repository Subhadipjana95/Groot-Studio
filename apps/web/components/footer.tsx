...
import { DiscordIcon } from "@workspace/ui/components/icons/icon"
import SectionDivider from "./utilities/SectionDivider"
import { usePathname } from "next/navigation"
import { cn } from "@workspace/ui/lib/utils"

const XIcon = ({ className }: { className?: string }) => (
  <svg viewBox="0 0 24 24" fill="currentColor" className={cn("h-4.5 w-4.5", className)} xmlns="http://www.w3.org/2000/svg">
  ...
  return (
    <footer className={cn("relative overflow-hidden w-full selection:bg-brand1/15 selection:text-brand1/75"
      ...
      <div className={cn("mx-auto relative max-w-96 border-x border-border",
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 py-2 pl-6 pr-3">
          ...
          <div className="hidden md:flex flex-wrap items-center justify-center md:justify-end gap-6 sm:gap-8">
            <div className="flex items-center gap-2">
              <a
                href="https://x.com/Subhadip53874"
                target="_blank"
                rel="noopener noreferrer"
                className="flex h-9 w-9 items-center justify-center rounded-md border bg-brand-color-1/5 cursor-pointer hover:bg-muted/80 transition-colors group"
              >
                <XIcon className="size-4.5 text-foreground grayscale-100 contrast-150 opacity-70 group-hover:grayscale-0 transition-all duration-150" />
              </a>
              <a
                href="https://discord.gg/5nJmZYA5f2"
                target="_blank"
                rel="noopener noreferrer"
                className="flex h-9 w-9 items-center justify-center rounded-md border bg-brand-color-1/5 cursor-pointer hover:bg-muted/80 transition-colors group"
              >
                <DiscordIcon className="size-4.5 text-foreground grayscale-100 contrast-150 opacity-70 group-hover:grayscale-0 transition-all duration-150" />
              </a>
              <a
                href="https://www.linkedin.com/in/subhadipjana095"
                target="_blank"
                rel="noopener noreferrer"
                className="flex h-9 w-9 items-center justify-center rounded-md border bg-brand-color-1/5 cursor-pointer hover:bg-muted/80 transition-colors group"
              >
                <LinkedinIcon className="size-4.5 text-foreground grayscale-100 contrast-150 opacity-70 group-hover:grayscale-0 transition-all duration-150" />
              </a>
            </div>
          </div>
