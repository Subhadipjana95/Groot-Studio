...
export function SponsorCard({ target = 4000, duration = 2000 }: { target?: number, duration?: number }) {
  ...
  return (
    <div className="group relative h-80 max-h-96 overflow-hidden rounded-xl bg-background shadow-lg transition-all border border-brand-color-3/20 isolate group">
      ...
      <div className="relative z-10 flex h-full flex-col p-5">
        ...
        <div className="mt-auto">
          ...
          <BrandButton href="/sponsors"
            label="Sponsor us"
            className="w-full mt-3 opacity-80 group-hover:opacity-100 transition-all duration-200 ease-in-out" />
        </div>
      </div>
    </div>
  );
